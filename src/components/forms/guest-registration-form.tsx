...
import { saveGuestRegistration } from "@/actions/guest-registration";
import { cities } from "@/constants/cities";
import { departments } from "@/constants/departments";
import { Input } from "../ui/input";
import { useRouter } from "next/navigation";

export default function GuestRegistrationForm({ guestRegistrations }: any) {
  const [error, setError] = useState<string | undefined>("");
  const [success, setSuccess] = useState<string | undefined>("");
  const [existingRegistration, setExistingRegistration] =
    useState<boolean>(false);
  const router = useRouter();
  const form = useForm<z.infer<typeof guestRegistrationSchema>>({
  ...
  async function onSubmit(values: z.infer<typeof guestRegistrationSchema>) {
    setSuccess("");
    setError("");
    if (existingRegistration) {
      setError("Registration already exists");
    } else {
      const result = await saveGuestRegistration(values);
      form.reset();
                    ...
                    <FormControl>
                      <Input
                        ...
                        onChange={(e: any) => {
                          ...
                          if (text.length > 5) {
                            const previousRecord = guestRegistrations.find(
                              (guest: any) => guest.cnic === text
                            );
                            if (previousRecord) {
